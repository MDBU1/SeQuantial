...
class ClassRefSequence(BaseModel):
    str_input_fasta: str
    _str_header: str
    _str_dna: str
    _int_length_dna: str
    _dict_composition_dna: dict

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self._str_header = self.meth_parse_fasta_header(self.str_input_fasta)
        self._str_dna = self.meth_parse_fasta_seq(self.str_input_fasta)
        self._int_length_dna = self.meth_calc_length_sequence(self._str_dna)
        self._dict_composition_dna = self.meth_calc_sequence_composition(self._str_dna)
        self.meth_calc_no_stop_codons_and_positions(self._str_dna)
    ...
    def meth_parse_fasta_header(self, seq) -> str:
        ...
        for seqID, line in enumerate(sequence):
            if line.startswith(">"):
                header = line.strip()
                logging.info(f"reference sequence loaded: " + header.replace(">", ""))
                return header
    ...
    def meth_parse_fasta_seq(self, seq) -> str:
        ...
        for seqID, line in enumerate(sequence):
            if line.startswith(">"):
                ...
                dna = line.strip("\n")
                logging.info(f"reference sequence: {dna}")
                return dna

    def meth_calc_length_sequence(self, seq) -> int:
        int_length_sequence = len(seq)
        logging.info(f"reference sequence length: {int_length_sequence}")
    ...
    def meth_calc_sequence_composition(self, seq) -> dict:
        ...
        for i in seq:
            dict_seq[i] += 1
        logging.info(f"sequence base composition: {dict_seq}")
        return dict_seq
    ...
    def meth_calc_no_stop_codons_and_positions(self, seq) -> dict:
        ...
        logging.debug(f"reference sequence as codons: {ref_seq_codons}")
        # print(ref_codon)
        ...
        logging.debug(f"reference sequence no stop codons: {list_int_stop_codons}")
